...
hourly_historic_data = {"date": pd.date_range(
	start = pd.to_datetime(hourly.Time(), unit = "s"),
	end = pd.to_datetime(hourly.TimeEnd(), unit = "s"),
	freq = pd.Timedelta(seconds = hourly.Interval()),
	inclusive = "left"
)}
hourly_historic_data["temperature_2m"] = hourly_temperature_2m
hourly_historic_data["relative_humidity_2m"] = hourly_relative_humidity_2m
hourly_historic_data["dew_point_2m"] = hourly_dew_point_2m
hourly_historic_data["apparent_temperature"] = hourly_apparent_temperature
hourly_historic_data["precipitation"] = hourly_precipitation
hourly_historic_data["rain"] = hourly_rain
hourly_historic_data["snowfall"] = hourly_snowfall
hourly_historic_data["snow_depth"] = hourly_snow_depth
hourly_historic_data["weather_code"] = hourly_weather_code
hourly_historic_data["pressure_msl"] = hourly_pressure_msl
hourly_historic_data["surface_pressure"] = hourly_surface_pressure
...
def filter_data(start_date, end_date):
    ...
    filtered_data = weather_data[(pd.to_datetime(weather_data['date']) >= start_date) & (pd.to_datetime(weather_data['date']) <= end_date)]
    ...
    if start_date < end_date:
       ...
       'soil_moisture_100_to_255cm']
        ...
        colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k', 'purple', 'orange', 'brown',
                 'pink', 'gray', 'olive', 'teal', 'navy', 'indigo', 'cyan', 'violet',
                 'lime', 'gold', 'silver', 'maroon', 'darkgreen', 'darkblue', 'darkred',
                 'darkorange', 'darkcyan', 'darkviolet']
        ...
        for i, (metric, color) in enumerate(zip(weather_metrics, colors)):
            ...
            for index, value in filtered_data[[metric, 'date']].iterrows():
                ax.text(value['date'], value[metric], round(int(value[metric]), 0), ha='left', va='bottom', fontsize=6)
